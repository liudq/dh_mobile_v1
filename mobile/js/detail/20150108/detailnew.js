...
Zepto(function($){
	...
	var DetailUtil={
		...
		getSkuId:function(){
			var skuInfo=[],skuidLen = $(".j-skuid").length,inpLen = $(".j-skuid").find('input').length;
			$(".j-skuSelect li.active").each(function(i,e){
				var $e=$(e);
				skuInfo.push($e.attr("attrvalid")+"-"+$e.attr("attrid"));
			});
			var obj=paramInfo.skuObj;
			var skumd5 =$("#J_skuLayer").attr("defaultskumd5");
			if(skumd5 ==="" || skumd5 ===null){
				skumd5 ="";
			}
			var sku=$("#J_skuLayer").attr("defaultskuid")+"_" + skumd5;
			//没有sku的情况做的判断skuidLen==1并且下面是没有input
			if(skuidLen==1 && inpLen==0){
				sku=obj['noattr'];
			}else{
				for(var k in obj){
					var ki= k.split("_");
		...
		getSkuObj:function(){
			var obj={},skuidLen = $(".j-skuid").length;
		...
		getDiv:function(){
			var $div=$("body").children("div:not(.dhm-hide),header,footer");
			var opt={};
				...
				success: function(data){
					if(data == null || data == undefined){
							...
							if(oSpan.length>0){
								...
								$("#J_shippingCty").attr('shipTypeId',oSpan);
...
//	cart buy
	...
	CartBuy.prototype={
		...
		getParam:function(){
			...
			// 	"&skumd5=",skuInfo[1]];
			// return data.join("");
			var quantity = paramInfo.quantity*1,
			 	proInfo = $('#J_proInfo'),
			 	supplierId = proInfo.attr('data-supplierId'),
			 	productId  = paramInfo.proid,
			 	unit = proInfo.attr('data-unit'),
			 	stockin = $("#stockin").val(),
			 	shipTypeId = $('#J_shippingCty').attr('shipTypeId');
		...
		buyNow:function(){
			...
			$buy.delegate($buy,'click',function(e){
				...
				//开启loading
                $loadingWarp.trigger('popupTip:loading', true);
				DHM.Common.request({
					url:self.buyUrl,
					type:"GET",
					dataType:'json',
		...
		buyNowRes:function(data,scope){
			...
			$popupWarp = $('.j-popup-warp');
				...
				if(data.state==='0x0000'){
	                ...
	                $popupWarp.trigger('popupTip:dataErrorTip', {action:'close',message:data.message});
	                //捕获异常
	                try {
		...
		fnError:function(url,scope,params){
			...
			$loadingWarp.trigger('popupTip:loading', false);
            //展示数据接口错误信息【点击ok，关闭层】
            $popupWarp.trigger('popupTip:dataErrorTip', {action:'close',message:'Network anomaly.'});
            //捕获异常
            try {
		...
		toCart:function(){
			...
			$toCart.delegate($toCart,'click',function(e){
				...
				if(!param) return;
				if(!self.isSkuSelected()) return;
				param.shipToCountry = paramInfo.ctyId;
				DHM.Common.request({
					url:self.cartUrl,
					data:param,
					async:false,
		...
		$.ajax({
			...
			success: function(data){
				...
				var tmpl = {
					...
					pro:'<a  href={{url}}/{{seoName}}/{{itemCode}}.html{{d1}} rel="nofollow"><img src={{imgUrl}} alt={{altName}}></a>',
					price:'<a href={{url}}/{{seoName}}/{{itemCode}}.html{{d1}} class="list-cont" rel="nofollow"><span class="price"><b class="mobile-deals"></b>{{currencyText}}{{discountPrice}}</span></a>'
				}
				if(language == 'RU'){
					tmpl = {
						warp:'<li>{{image}}{{price}}</li>',
						...
						promo:'<span class={{icon}}>{{discountRate}}</span>',
						pro:'<a  href={{url}}/{{seoName}}/{{itemCode}}.html{{d1}} rel="nofollow"><img src={{imgUrl}} alt={{altName}}></a>',
						price:'<a href={{url}}/{{seoName}}/{{itemCode}}.html{{d1}} class="list-cont" rel="nofollow"><span class="price"><b class="mobile-deals"></b>{{discountPrice}}{{currencyText}}</span></a>'
					}
				}
				if(state=="0x0000"){
					if(len>0){
						for(var i=0;i<len;i++){
							...
							if (dd.promoType=="0") { //折扣
								...
								price = tmpl.price.replace(/\{\{url\}\}/,url).replace(/\{\{seoName\}\}/,dd.seoName).replace(/\{\{itemCode\}\}/,dd.itemCode).replace(/\{\{currencyText\}\}/,data.data.currencyText).replace(/\{\{discountPrice\}\}/,dd.discountPrice);
								...
								if(dd.discountRate!==''){
									...
									price = tmpl.price.replace(/\{\{url\}\}/,url).replace(/\{\{seoName\}\}/,dd.seoName).replace(/\{\{itemCode\}\}/,dd.itemCode).replace(/\{\{currencyText\}\}/,data.data.currencyText).replace(/\{\{discountPrice\}\}/,dd.maxPrice);
								}
							...
							pro = tmpl.pro.replace(/\{\{url\}\}/,url).replace(/\{\{seoName\}\}/,dd.seoName).replace(/\{\{itemCode\}\}/,dd.itemCode).replace(/\{\{imgUrl\}\}/,dd.imgUrl).replace(/\{\{altName\}\}/,dd.seoName);
                            
                            //D1推荐曝光打标
                            if (!dd.D1Tag) {
                                price = price.replace(/\{\{d1\}\}/,'#cppd-'+(i+1)+'-9|null:01');
                                pro = pro.replace(/\{\{d1\}\}/,'#cppd-'+(i+1)+'-9|null:01');
                            } else {
                               price = price.replace(/\{\{d1\}\}/,'#'+dd.D1Tag);
                               pro = pro.replace(/\{\{d1\}\}/,'#'+dd.D1Tag);
                            }
