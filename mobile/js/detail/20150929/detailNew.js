...
Zepto(function($){
	...
	var defInventoryQty ="",
		...
		isinventory="",//是否为备货商品 
		istate="",//是否售空
		ntalker_js_url="", //ntalker
		ntalkerSellerid ="",//ntalkerSellerId
		ntalker_buyerid ="",
		htmlurl ="",
		prodLineId="";
	//引入调用入口
	...
	var impressionInfo = window.location.hash.substr(1);
	iminfo = impressionInfo;
	...
	var DetailUtil={
		...
		getPrice:function(callback){
			var self = this;
				dataJson = JSON.parse(decodeURIComponent($($('#J_sku')).attr("dataValue")));
				...
				ntalkerSellerid = dataJson.ntalker.ntalker_sellerid;
				ntalker_buyerid = dataJson.ntalker.ntalker_buyerid;
				prodLineId = dataJson.prodLineId;

				if(dataJson.promoDto !=undefined){
				...
				if(value ==1 && dataJson.itemVipDto!=null && dataJson.promoDto !=undefined){
					...
					if(lot >1){
						$('.sku_price').html('<strong>US '+'$'+dataJson.itemVipDto.minVipPrice+' - '+dataJson.itemVipDto.maxVipPrice+'</strong> / <span class="smallPrice" style="display:none;">Lot</span><span class="smallPrices">Lot&nbsp;&nbsp;('+lot+'&nbsp;'+dataJson.plural+' / Lot)</span>');
						$('#line_price').html('US&nbsp;' +'$'+dataJson.minPrice + ' - ' + dataJson.maxPrice + ' / ' +'Lot');
						...
						$('.sku_price').html('<strong>US '+'$'+dataJson.itemVipDto.minVipPrice+' - '+dataJson.itemVipDto.maxVipPrice+'</strong> / <span class="smallPrice">'+measureName+'</span>');
						$('#line_price').html('US&nbsp;' +'$'+dataJson.minPrice + ' - ' + dataJson.maxPrice + ' / '  +measureName);	
					...
					if(lot >1){
						if(itemVipDto.minVipPrice ==  itemVipDto.maxVipPrice && minPrice ==maxPrice){
							$('.sku_price').html('<strong>US '+'$'+itemVipDto.minVipPrice+'</strong> / <span class="smallPrice" style="display:none;">Lot</span><span class="smallPrices">Lot&nbsp;&nbsp;('+lot+'&nbsp;'+dataJson.plural+' / Lot)</span>');
							$('#line_price').html('US&nbsp;' +'$'+minPrice +  ' / '  +'Lot');
						}else{
							$('.sku_price').html('<strong>US '+'$'+itemVipDto.minVipPrice+' - '+itemVipDto.maxVipPrice+'</strong> / <span class="smallPrice" style="display:none;">Lot</span><span class="smallPrices">Lot&nbsp;&nbsp;('+lot+'&nbsp;'+dataJson.plural+' / Lot)</span>');
							$('#line_price').html('US&nbsp;' +'$'+minPrice + ' - ' + maxPrice + ' / '  +'Lot');
						}
						$('.j-priceUnit').html(lot+'&nbsp;'+dataJson.plural+' / Lot');
						var quantityVal = $('#J_quantity').val() > 1?'Lots':"Lot";
							$('.j-companyInfo').html(quantityVal);
						...
						if(itemVipDto.minVipPrice ==  itemVipDto.maxVipPrice && minPrice ==maxPrice){
							$('.sku_price').html('<strong>US '+'$'+itemVipDto.minVipPrice+'</strong> / <span class="smallPrice">'+measureName+'</span>');
							$('#line_price').html('US&nbsp;' +'$'+minPrice + ' / '  +measureName);
						}else{
							$('.sku_price').html('<strong>US '+'$'+itemVipDto.minVipPrice+' - '+itemVipDto.maxVipPrice+'</strong> / <span class="smallPrice">'+measureName+'</span>');
							$('#line_price').html('US&nbsp;' +'$'+minPrice + ' - ' + maxPrice + ' / ' +measureName);
						}
						var quantityVal = $('#J_quantity').val() > 1?dataJson.plural:measureName;
							$('.j-companyInfo').html(quantityVal);
							$('.j-unitInfo').html('Limited to <span class="j-inventory">'+promoLimitPurchaseNum+'</span><span class="j-company dhm-hide">'+dataJson.plural+'</span><span class="j-companyShow">&nbsp;'+quantityVal+'</span>&nbsp;per Buyer');
				...
				else if(dataJson.promoDto !=undefined){
					...
					if(lot > 1){
						if(minPrice ==maxPrice && promoMinPrice == promoMaxPrice){
							$('#line_price').html('US &nbsp;' + '$'+minPrice + ' / '+'Lot');
							$('.sku_price').html('<strong>US '+'$'+promoMinPrice+'</strong> / <span class="smallPrice" style="display:none;">Lot</span><span class="smallPrices">Lot&nbsp;&nbsp;('+lot+'&nbsp;'+dataJson.plural+' / Lot)</span>');
						}else{
							$('#line_price').html('US ' + '$'+minPrice + ' - ' + maxPrice + ' / '  +'Lot');
							$('.sku_price').html('<strong>US '+'$'+promoMinPrice+' - '+promoMaxPrice+'</strong> / <span class="smallPrice" style="display:none;">Lot</span><span class="smallPrices">Lot &nbsp;&nbsp;('+lot+'&nbsp;'+dataJson.plural+' / Lot)</span>');
						}
						$('.j-priceUnit').html(lot+'&nbsp;'+dataJson.plural+' / Lot');
						var quantityVal = $('#J_quantity').val() > 1?'Lots':"Lot";
						$('.j-companyInfo').html(quantityVal);
						...
						if(minPrice ==maxPrice && promoMinPrice == promoMaxPrice){
							$('#line_price').html('<span>US </span>$'+minPrice+' / '+measureName+'')
							$('.sku_price').html('<strong>US '+'$'+promoMinPrice+'</strong> / <span class="smallPrice">'+measureName+'</span>');	
						}else{
							$('#line_price').html('<span>US </span>$'+minPrice+' - '+maxPrice+' / '+measureName+'')
							$('.sku_price').html('<strong>US '+'$'+promoMinPrice+' - '+promoMaxPrice+'</strong> / <span class="smallPrice">'+measureName+'</span>');
						}
						var quantityVal = $('#J_quantity').val() > 1?dataJson.plural:measureName;
							$('.j-companyInfo').html(quantityVal);
							$('.j-unitInfo').html('Limited to <span class="j-inventory">'+promoLimitPurchaseNum+'</span><span class="j-company dhm-hide">'+dataJson.plural+'</span><span class="j-companyShow">&nbsp;'+quantityVal+'</span>&nbsp;per Buyer');
					...
					if(lot > 1){
						if(minPrice == maxPrice){
							$('.sku_price').html('<strong>US '+'$'+minPrice+'</strong> / <span class="smallPrice" style="display:none;">Lot</span><span class="smallPrices">Lot&nbsp;&nbsp;('+lot+'&nbsp;'+dataJson.plural+' / Lot)</span>');
						}else{
							$('.sku_price').html('<strong>US '+'$'+minPrice+' - '+maxPrice+'</strong> / <span class="smallPrice" style="display:none;">Lot</span><span class="smallPrices">Lot&nbsp;&nbsp;('+lot+'&nbsp;'+dataJson.plural+' / Lot)</span>');
							$('.j-priceUnit').html(lot+'&nbsp;'+dataJson.plural+' / Lot');
						}
						$("#J_quantity").val(dataJson.minOrder)	;
						var quantityVal = $('#J_quantity').val() > 1?'Lots':"Lot";
						...
						if(minPrice == maxPrice){
							$('.sku_price').html('<strong>US '+'$'+minPrice+'</strong> / <span class="smallPrice">'+measureName+'</span>');
						}else{
							$('.sku_price').html('<strong>US '+'$'+minPrice+' - '+maxPrice+'</strong> / <span class="smallPrice">'+measureName+'</span>');
						}
						var quantityVal = $('#J_quantity').val() > 1?dataJson.plural:measureName;
							$('.j-companyInfo').html(quantityVal);
							$('.j-unitInfo').html('Limited to <span class="j-inventory">'+promoLimitPurchaseNum+'</span><span class="j-company dhm-hide">'+dataJson.plural+'</span><span class="j-companyShow">&nbsp;'+quantityVal+'</span>&nbsp;per Buyer');
		...
		translate:function($d,dist,flag,f1){
			...
			if(dist==0){
				...
				setTimeout(function(){
					...
					$d.css({"position":"absolute","height":h-100,"display":"block"});
				},0);
		...
		getSkuallmatch:function(a,b,$e){    //当页面初始化时，，遍历第一个属性。
			...
			var test_sku = "";
				...
				for( var k=0 ;k<length.length;k++){  //属性下面有几个就遍历即便
					...
					for(var j = 0 ;j<itemAttrListSku.length;j++){
						...
						if(test_sku==itemSkuRelAttrall){
							if(skuSaleStatus==1) {
								...
								$($($('.j-skuSelect')[a[a.length-1]]).find("li")[k]).css({"background-color":"rgb(235,234,228)","border":"1px solid #cbc8bb","color":"#999","opacity":"0.3"});
					...
					if(state==true){
						$($($('.j-skuSelect')[a[a.length-1]]).find("li")[k]).css({"background-color":"rgb(255, 255, 255)","color":"#333","opacity":"10"});
			...
			if(a.length==itemAttrListSkulenght.length-1){
				...
				var lastSku=lastnumlenght[t].attrValueId;
					...
					for(var j = 0 ;j<itemAttrListSku.length;j++){
						...
						if(test_sku==itemSkuRelAttrall){
							if(skuSaleStatus==1) {
								...
								$($($('.j-skuSelect')[sku_num]).find("li")[t]).css({"background-color":"rgb(235,234,228)","border":"1px solid #cbc8bb","color":"#999","opacity":"0.3"});
							}
						}
						itemSkuRelAttrall =""	;
						position_i =[];
					...
					if(state==true){
						$($($('.j-skuSelect')[sku_num]).find("li")[t]).css({"background-color":"rgb(255, 255, 255)","color":"#333","opacity":"10"});
						...
						for(var j = 0 ;j<itemAttrListSku.length;j++){
							...
							if(test_sku==itemSkuRelAttrsingle){
								if(skuSaleStatus==1) {
									...
									$($($('.j-skuSelect')[h]).find("li")[l]).css({"background-color":"rgb(235,234,228)","border":"1px solid #cbc8bb","color":"#999","opacity":"0.3"});
								}
							}
							itemSkuRelAttrsingle ="";
							if(state==true){
								$($($('.j-skuSelect')[h]).find("li")[l]).css({"background-color":"rgb(255,255,255)","color":"#333","opacity":"10"});
							}
							state = false;
						}		
					}
...
//	轮播图 urlHash='#viewpic'
	...
	Description.prototype={
		...
		getDes:function(){
			var self=this;
				descriptionValue = $('#J_descriptionLayer').html().trim();
				if(descriptionValue == ""){
					DHM.Common.request({
						url:self.url,
						type:"GET",
	...
	ShowLayer.prototype={
		...
		show:function(){
			...
			$.each(obj,function(i,o){
				...
				//$('body').delegate("nav .j-back,.ceng","click",function(){
				$('nav .j-back,.ceng').unbind('click').bind('click',function(){
	...
	DetailSKU.prototype={
		...
		sku:function(){
			...
			self.$con.delegate('.j-skuSelect li','click',function(){
				...
				if($e.css("background-color")=="rgb(235,234,228)") return;
				$e.addClass("active");
...
//	cart buy
	...
	CartBuy.prototype={
		...
		buyNowAjax:function(){
			...
			if(itemAttrListSkulenght==undefined){
				var skuid = itemAttrListSku[0].skuId,
					skumd5 = itemAttrListSku[0].skuMd5,
					stockCountry = itemAttrListSku[0].inventoryLocation;
		...
		getParam:function(){
			...
			var skuMd5 = $('#J_sku').attr("skumd5"),
				skuid =$('#J_sku').attr("skuid"),
				shipToCountry = $('.j-shipcountry').attr('vname'),
				shipType = $('.j-shipway').attr("data-expresstype"),
				quantity = $('#J_quantity').val(),
				data = {
					quantity:quantity,
					productId:productId,
		...
		toCartAdd:function(params){
			...
			DHM.Common.request({
					...
					data:$.extend(true, {}, params[1], {
						'shipToCountry':$('.j-shipcountry').attr('vname'),
						'shipTypeId':$('.j-shipway').attr("data-expresstype"),
						'stockIn':stockcountry,
						"supplierId":supplierid,
						"unit":measureName,
						"impressionInfo":iminfo,
                            ...
                            for(var i=0;i<len;i++){
                                ...
                                if(dd.promoType=="0"){
                                    html=['<li>',
                                        ...
                                        '<a href=',url,'/' + dd.seoName + '/',dd.itemCode,'.html',d1,' rel="nofollow"><img src=',dd.imgUrl,'  alt=',dd.seoName,'></a>',
                                        '</div>',
                                        '<a href=',url,'/' + dd.seoName + '/',dd.itemCode,'.html',d1,' class="list-cont" rel="nofollow">',
                                ...
                                //直降
                                    ...
                                    html=['<li>',
                                        ...
                                        '<a href=',url,'/' + dd.seoName + '/',dd.itemCode,'.html',d1,' rel="nofollow"><img src=',dd.imgUrl,' alt=',dd.seoName,'></a>',
                                        '</div>',
                                        '<a href=',url,'/' + dd.seoName + '/',dd.itemCode,'.html',d1,' rel="nofollow" class="list-cont">',
...
//	tracking
	function tracking(){
		...
		$("#nav_desLayer,#J_des").click(function(){
