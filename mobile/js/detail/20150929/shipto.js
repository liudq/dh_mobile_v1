   		...
   		Shipto.prototype = {
	                ...
	                success: function(res){
	                    if (res.data && res.state === '0x0000') {
	                    	if(res.data.shipCostAndWay==''){
	                    		...
	                    		if($('.j-shiptobtn').attr('issold')!=0 && !$('.addCart,.J_buyNow,.J_addToCart').hasClass('disableAddCart')){
	                    			$('.addCart').addClass('disableAddCart');
	                    			$('.J_buyNow,.J_addToCart').addClass('noClick');
	                    		}
	                    		$('.js-notcanship').attr("noShipTo","true")//标记该商口不能运达
	        ...
	        renderdefault:function(data){
    		   ...
    		   if(data.shipCostAndWay.shipcost==0){
            		...
            		$('.js-notcanship').attr("noShipTo","false")//标记该商口不能运达
            	}else{
            		$('.js-notcanship').attr("noShipTo","false")//标记该商口不能运达
            		this.shipcost.addClass('hasprice');
            		this.shipcost.html('<span>$'+data.shipCostAndWay.shipcost+'</span>Shipping');
            	}
            	var leadingTime = data.shipCostAndWay.leadingTime,
            		num =leadingTime > 1?leadingTime+"&nbsp"+"business days":leadingTime +"&nbsp"+"business day";
            	...
            	this.shipway.attr("data-expressType",data.shipCostAndWay.expressType);
            	this.deliverytm.html(data.shipCostAndWay.lowerDate+ '&nbsp;and&nbsp;'+ data.shipCostAndWay.upperDate);
            	this.shipwithin.html(num);
	        ...
	        //stockin 初始化
	        stockin:function(flag, callback, callbackParams){
	        ...
	        shiptoLayyer:function(evt){
	        	...
	        	var _self = this,
	        		...
	        		toCart = evt.data&&evt.data.toCart||false,
	        		toCartRun = evt.data&&evt.data.toCartRun||false,
	        		toCartParams = evt.data&&evt.data.params||false;

	        	target.attr('vtop',top);
	        	this.shiplayer.css({'top':'100px'});
	        	this.stockin(toCart, toCartRun, toCartParams);
	        	//this.shipmethod();
	        ...
	        shipmethod:function(flag){
	        	...
	        	var _self = this,skuid = this.sku.attr('skuid'),stockCountryParma = $('#J_sku').attr('stockcountry');
	        	if(!flag){
	        	...
	        	if(!stockCountryParma){
	        		stockCountryParma = this.shipcountry.attr('vstockname');
	        	}
				if(!skuid){
					var dataParams={
		        		"client":"wap",
		        		...
		        		"stockCountry":stockCountryParma
		        	};
				}else{
					var dataParams={
		        		"client":"wap",
	        	...
	        	if (!this.getCache()) {
		                ...
		                success: function(res){
		                    ...
		                    if (res.data && res.state === '0x0000') {
		                    	if(res.data.length==0){
					         		...
					         		$('.re-delivery').hide();
					         		$('.js-notcanship').attr("noShipTo","true")//标记该商口不能运达
					         		var noShipTo  =$('.js-notcanship').attr("noShipTo");
					         		//console.log($('.j-shiptobtn').attr('issold')!=0 && !$('.addCart,.J_buyNow,.J_addToCart').hasClass('disableAddCart')|| noShipTo=="true")
			                		if($('.j-shiptobtn').attr('issold')!=0 && !$('.addCart,.J_buyNow,.J_addToCart').hasClass('disableAddCart')|| noShipTo=="true"){
		                    			$('.addCart').addClass('disableAddCart');
		                    			$('.J_buyNow,.J_addToCart').addClass('noClick');
		                    		}
					         		...
					         		$('.js-notcanship').attr("noShipTo","false")//标记该商口不能运达
					         		var noShipTo  =$('.js-notcanship').attr("noShipTo");
					         		if($('.j-shiptobtn').attr('issold')!=0 && $('.addCart,.J_buyNow,.J_addToCart').hasClass('disableAddCart') || noShipTo == "false"){
		                    			$('.addCart').removeClass('disableAddCart');
		                    			$('.J_buyNow,.J_addToCart').removeClass('noClick');
		                    		}
					         	}
	        		...
	        		if($('.cannot').length){
	        				...
	        				this.notcanship.show();
			         		...
			         		$('.js-notcanship').attr("noShipTo","true")//标记该商口不能运达
					        var noShipTo  =$('.js-notcanship').attr("noShipTo");
			         		if($('.j-shiptobtn').attr('issold')!=0 && !$('.addCart').hasClass('disableAddCart') || noShipTo == "true"){
                    			$('.addCart').addClass('disableAddCart')
                    		}
			         	}else{
					        ...
					        var noShipTo  =$('.js-notcanship').attr("noShipTo");
			         		if($('.j-shiptobtn').attr('issold')!=0){
		                		if($('.j-shiptobtn').attr('issold')!=0 && $('.addCart').hasClass('disableAddCart') || noShipTo == "false"){
	                    			$('.addCart').removeClass('disableAddCart')
	                    		}
		                	}
			         	}
	        ...
	        getParams: function(target,index){
	        	var param = {},itemcode = this.itemcode,vname = target.attr('vname'),skuId = this.sku.attr('skuid'),stockCountryParma = $('#J_sku').attr('stockcountry');
	        	if(!this.sku.attr('vquantity')){
	        		var vquantity = this.minOrder;
	        	}else{
	        		vquantity = this.sku.attr('vquantity');
	        ...
	        passVal:function(){
            	...
            	var dataShipway= activeLi.find('.js-m-shipway').attr('data-expresstype');
	        	...
	        	this.shipway.attr('data-expresstype',dataShipway);
