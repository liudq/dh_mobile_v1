...
Zepto(function($) {
	...
	function endActiveEgg() {
		...
		$.getJSON(countUrl, function(data) {
			if (!data) return;
			var endTime = new Date("2015/04/05 23:00:00").getTime(); //活动结束时间(北京时间)
	...
	function setEasterEggItems(data) {
		...
		for (var i = 0; i < len; i++) {
			...
			if (dd.isEgg == "true") {
				var html = ['<li eggtag=', url, '#lp_easterpro-wap-', dd.itemcode, ' class="J-EggItems">',
					'<div class="list-img">',
					'<a href=', url, '#lp_easterpro-wap-', dd.itemcode, ' onclick="return false"><img src=', dd.imageurl, ' /></a>',
					'</div>',
					'<a href=', url, '#lp_easterpro-wap-', dd.itemcode, ' onclick="return false" class="list-cont">',
				...
				var html = ['<li>',
					'<div class="list-img">',
					'<a href=', url, '#lp_easterpro-wap-', dd.itemcode, '><img src=', dd.imageurl, ' /></a>',
					'</div>',
					'<a href=', url, '#lp_easterpro-wap-', dd.itemcode, ' class="list-cont">',
					'<span class="text-black">', dd.title, '</span>',
					'</a>',
					'</li>'
				];
