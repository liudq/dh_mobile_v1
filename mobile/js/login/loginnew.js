...
//    与一些组件相关的js
    ...
    DHMLogin.prototype={
        ...
        changeTab:function(){
            var self=this,
                buyername=DHM.Cookie.getCookie('m_login_name'),
                $tit = $(self.opt.titId),
                $logreg = $(self.opt.logreg),
                $guestmode = $(self.opt.guestmode);
               
            if(!buyername || buyername===''){
                $guestmode.show();
                $tit.eq(1).addClass("cur-bg").siblings('.tit').removeClass("cur-bg");
                $logreg.find('.sec').eq(1).addClass("mar-b-30").show();
                $logreg.find('.sec').eq(1).siblings(".sec").removeClass("mar-b-30").hide()
            }else{
                $guestmode.attr('oldUser',true)
                $guestmode.hide();
                $('#J_login').find('.emailadd').val(buyername);
            }
            if(!$(self.opt.formId).get(0)) return;
            
            $tit.on("click",function(){
                var index = $(this).index(),targetTabcon = $logreg.find('.sec').eq(index);
                $(this).addClass("cur-bg").siblings('.tit').removeClass("cur-bg");
                targetTabcon.addClass("mar-b-30").show();
                targetTabcon.siblings(".sec").removeClass("mar-b-30").hide();
                if($guestmode.attr('oldUser')){
                    if(index==0){$guestmode.hide();}else{$guestmode.show();}
                }else{
                    $guestmode.show();
                }
